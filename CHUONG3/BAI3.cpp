...
int main() {
	...
	for (int i = 1; i <= list1.getSize(); i++) {
		int temp = list1.phanTuThuK(i)->getData();
		bool check = false;
		for (int j = 1; j <= list2.getSize(); j++) {
			int temp2 = list2.phanTuThuK(j)->getData();
			int iTemp = i;
			int iTemp2 = j;
				while (temp == temp2 && iTemp <= list1.getSize() && iTemp2 <= list2.getSize()) {
	
					list3.addTail(temp);
					iTemp++;
					iTemp2++;
					if (iTemp > list1.getSize() || iTemp2 > list2.getSize()) {
						break;
					}
					temp = list1.phanTuThuK(iTemp)->getData();
					temp2 = list2.phanTuThuK(iTemp2)->getData();
